...
export async function addToCart({
  ...
  quantity: number
  countryCode: string
  /** Ex. linia de garanție extinsă poartă produsul pe care îl acoperă. */
...
export async function initiatePaymentSession(
  ...
  return sdk.store.payment
    ...
    .then(async (resp) => {
      // Stripe filters country-restricted methods (Klarna, Afterpay) when the
      // PaymentIntent has no buyer context. Push the cart's shipping address
      // and email to the freshly created PaymentIntent so eligibility is
      // re-evaluated before the Payment Element loads.
      try {
        await sdk.client.fetch(`/store/carts/${cart.id}/stripe-enrich`, {
          method: "POST",
          headers,
        })
      } catch {
        // Non-fatal: the user can still pay; only country-restricted methods
        // may be hidden if enrichment fails.
      }
      const cartCacheTag = await getCacheTag("carts")
      revalidateTag(cartCacheTag)
      return resp
